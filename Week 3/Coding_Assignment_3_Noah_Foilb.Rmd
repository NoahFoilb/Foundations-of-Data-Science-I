...
```{r}
#Using Glimpse we get more information about SW
glimpse(SW)

# There are three numerical variables, Eight Character variables, and No caregorical varaibles (UNless its "List,which there would be three)
...
SW1 <- SW %>%
  select(name,height,mass,hair_color,skin_color,eye_color,birth_year,sex,gender,homeworld,species)  # Use select to which variables to keep

head(SW1,10)  #Display top ten values
```
...
```{r}
SWyellow <- SW1 %>%
  filter(eye_color == "yellow" | skin_color == "yellow" ) %>%
  select(name,eye_color, skin_color, homeworld, species)
...
```{r}
table(SW1$hair_color,SW1$eye_color)

# there is 9 people with no eye color and black hair. 9 with brown eyes and black hair, 9 people with brown eyes and brown hair.
```
...
```{r}
SWstats <- SW1 %>%
  mutate(name,height,mass,species,BMI = (mass/(height/100)**2))
...
```{r}
SWstats$BMI <- round(SWstats$BMI,1)
head(SWstats)
```
...
```{r}
SWstats2$bmi_rating <- if_else(SWstats2$BMI>100, "Obese",
          if_else(SWstats2$BMI>=30, "Heavy","Normal"))
